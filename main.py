...
app_state = {
    ...
    "prediction_num": 0,
    "last_result": "Establishing Session..."
}
...
def background_worker():
    ...
    # Session create karte hain taaki cookies maintain rahein
    session = requests.Session()
    
    # Pehle site visit karke session establish karte hain
    try:
        session.get(REFERER_URL, headers={"User-Agent": HEADERS["User-Agent"]}, timeout=10)
    ...
    while True:
        try:
            ...
            if response.status_code == 200:
                ...
                # Agar 403 aaye toh session refresh karne ki koshish karo
                try:
                    session.get(REFERER_URL, headers={"User-Agent": HEADERS["User-Agent"]}, timeout=5)
                except:
                    pass
        except Exception as e:
